...
const SshKeyRowValue = styled(
  'div',
  'truncate text-text-black text-text-medium'
);
const SshKeyAddedAt = styled(
  'div',
  'truncate text-text-grey text-text-regular'
);

const SshKeyRow: Component<{ key: UserKey; refetchKeys: () => void }> = (
  props
) => {
    ...
    <>
      <div class="flex w-full items-center gap-2 overflow-x-hidden bg-ui-primary px-5 py-4 last:border-b-none">
        <div class="flex w-full flex-col overflow-x-hidden">
          <SshKeyName>
            {props.key.name === '' ? '(Name not set)' : props.key.name}
          </SshKeyName>
          ...
          >
            <SshKeyAddedAt>
              Added on {dateHuman(props.key.createdAt!)}
            </SshKeyAddedAt>
          </Show>
        </div>
      ...
      <Modal.Container>
        ...
        <Modal.Body>
          <ModalDeleteConfirm>
            <div class="flex w-full flex-col">
              ...
              <Show
                when={
                  props.key.createdAt && props.key.createdAt?.seconds !== 0n
                }
                fallback={<SshKeyAddedAt>Added on ----</SshKeyAddedAt>}
              >
                <SshKeyAddedAt>
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            size="medium"
            onClick={() => handleDeleteKey(props.key.id)}
          >
            Yes, Delete
          </Button>
        </Modal.Footer>
...
const SshKeys: Component<{ keys: UserKey[]; refetchKeys: () => void }> = (
  props
) => {
  return (
    <List.Container>
      <For each={props.keys}>
        {(key) => <SshKeyRow key={key} refetchKeys={props.refetchKeys} />}
      </For>
    </List.Container>
  );
};
...
export default () => {
  const [userKeys, { refetch: refetchKeys }] = createResource(() =>
    client.getUserKeys({})
  );
  const {
    Modal: AddNewKeyModal,
    open: newKeyOpen,
    close: newKeyClose,
  } = useModal();
  ...
  const handleSubmit: SubmitHandler<
    PlainMessage<CreateUserKeyRequest>
  > = async (values) => {
    try {
          ...
          <AddNewKeyModal.Body>
            <div class="flex w-full flex-col gap-2">
              ...
              <Field
                of={formStore}
                name="publicKey"
                validate={[required('Enter SSH Public Key')]}
              >
                {(field, fieldProps) => (
                  <TextField
                    label="Key"
