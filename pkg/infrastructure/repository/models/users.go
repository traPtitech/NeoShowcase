...
type userR struct {
	...
	UserKeys     UserKeySlice     `boil:"UserKeys" json:"UserKeys" toml:"UserKeys" yaml:"UserKeys"`
}
...
func (r *userR) GetApplications() ApplicationSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetUserKeys() UserKeySlice {
	if r == nil {
		return nil
	}
	return r.UserKeys
...
var (
	userAllColumns            = []string{"id", "name", "admin"}
	userColumnsWithoutDefault = []string{"id", "name", "admin"}
...
func (o *User) UserKeys(mods ...qm.QueryMod) userKeyQuery {
	...
	return UserKeys(queryMods...)
}

// LoadApplications allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadApplications(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("`applications`.`id`, `applications`.`name`, `applications`.`repository_id`, `applications`.`ref_name`, `applications`.`commit`, `applications`.`deploy_type`, `applications`.`running`, `applications`.`container`, `applications`.`current_build`, `applications`.`current_commit`, `applications`.`want_commit`, `applications`.`created_at`, `applications`.`updated_at`, `a`.`user_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.RepositoryID, &one.RefName, &one.Commit, &one.DeployType, &one.Running, &one.Container, &one.CurrentBuild, &one.CurrentCommit, &one.WantCommit, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
...
func (userL) LoadUserKeys(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_keys`),
		qm.WhereIn(`user_keys.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetApplications removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Applications accordingly.
...
func (o *User) SetApplications(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Application) error {
	...
	removeApplicationsFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Applications = nil
	}

	return o.AddApplications(ctx, exec, insert, related...)
}

// RemoveApplications relationships from objects passed in.
// Removes related items from R.Applications (uses pointer comparison, removal does not keep order)
...
func (o *User) RemoveApplications(ctx context.Context, exec boil.ContextExecutor, related ...*Application) error {
	...
	for _, rel := range related {
		for i, ri := range o.R.Applications {
			if rel != ri {
				continue
			}

			ln := len(o.R.Applications)
			if ln > 1 && i < ln-1 {
				o.R.Applications[i] = o.R.Applications[ln-1]
			}
			o.R.Applications = o.R.Applications[:ln-1]
			break
		}
	}
