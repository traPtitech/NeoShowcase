...
func testUserToManyAppApplications(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into `owners` (`user_id`, `app_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `owners` (`user_id`, `app_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.AppApplications().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAppApplications(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AppApplications); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AppApplications = nil
	if err = a.L.LoadAppApplications(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AppApplications); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAppApplications(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAppApplications(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AppApplications[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AppApplications[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AppApplications().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpAppApplications(t *testing.T) {
	var err error
	...
	err = a.SetAppApplications(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAppApplications(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.AppApplications[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.AppApplications[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpAppApplications(t *testing.T) {
	var err error
	...
	err = a.AddAppApplications(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAppApplications(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.AppApplications) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.AppApplications[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.AppApplications[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `char`, `Name`: `varchar`}
	_           = bytes.MinRead
)
