...
var UserRels = struct {
	AppApplications string
}{
	AppApplications: "AppApplications",
}

// userR is where relationships are stored.
type userR struct {
	AppApplications ApplicationSlice `boil:"AppApplications" json:"AppApplications" toml:"AppApplications" yaml:"AppApplications"`
}
...
func (r *userR) GetAppApplications() ApplicationSlice {
	if r == nil {
		return nil
	}
	return r.AppApplications
}
...
// AppApplications retrieves all the application's Applications with an executor via id column.
func (o *User) AppApplications(mods ...qm.QueryMod) applicationQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`owners` on `applications`.`id` = `owners`.`app_id`"),
...
// LoadAppApplications allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAppApplications(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("`applications`.`id`, `applications`.`repository_id`, `applications`.`created_at`, `applications`.`updated_at`, `applications`.`deleted_at`, `a`.`user_id`"),
		qm.From("`applications`"),
		qm.InnerJoin("`owners` as `a` on `applications`.`id` = `a`.`app_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.RepositoryID, &one.CreatedAt, &one.UpdatedAt, &one.DeletedAt, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for applications")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice applications")
		}
	...
	if singular {
		object.R.AppApplications = resultSlice
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.AppApplications = append(local.R.AppApplications, foreign)
				if foreign.R == nil {
					foreign.R = &applicationR{}
				}
...
// AddAppApplications adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AppApplications.
// Sets related.R.Users appropriately.
func (o *User) AddAppApplications(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Application) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into `owners` (`user_id`, `app_id`) values (?, ?)"
	...
	if o.R == nil {
		o.R = &userR{
			AppApplications: related,
		}
	} else {
		o.R.AppApplications = append(o.R.AppApplications, related...)
	}
...
// SetAppApplications removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's AppApplications accordingly.
// Replaces o.R.AppApplications with related.
// Sets related.R.Users's AppApplications accordingly.
func (o *User) SetAppApplications(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Application) error {
	...
	removeAppApplicationsFromUsersSlice(o, related)
	if o.R != nil {
		o.R.AppApplications = nil
	}

	return o.AddAppApplications(ctx, exec, insert, related...)
}

// RemoveAppApplications relationships from objects passed in.
// Removes related items from R.AppApplications (uses pointer comparison, removal does not keep order)
// Sets related.R.Users.
func (o *User) RemoveAppApplications(ctx context.Context, exec boil.ContextExecutor, related ...*Application) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from `owners` where `user_id` = ? and `app_id` in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.AppApplications {
			if rel != ri {
				continue
			}

			ln := len(o.R.AppApplications)
			if ln > 1 && i < ln-1 {
				o.R.AppApplications[i] = o.R.AppApplications[ln-1]
			}
			o.R.AppApplications = o.R.AppApplications[:ln-1]
			break
		}
	}
...
func removeAppApplicationsFromUsersSlice(o *User, related []*Application) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
