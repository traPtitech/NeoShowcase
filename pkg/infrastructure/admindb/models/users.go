...
func (userL) LoadApplications(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`applications`.`id`, `applications`.`name`, `applications`.`repository_id`, `applications`.`ref_name`, `applications`.`deploy_type`, `applications`.`running`, `applications`.`container`, `applications`.`current_commit`, `applications`.`want_commit`, `applications`.`created_at`, `applications`.`updated_at`, `a`.`user_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.RepositoryID, &one.RefName, &one.DeployType, &one.Running, &one.Container, &one.CurrentCommit, &one.WantCommit, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
