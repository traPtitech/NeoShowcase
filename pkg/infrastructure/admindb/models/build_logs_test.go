...
func testBuildLogToOneBuildStatusUsingResultBuildStatus(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.Result = foreign.Status
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ResultBuildStatus().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := BuildLogSlice{&local}
	if err = local.L.LoadResultBuildStatus(ctx, tx, false, (*[]*BuildLog)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ResultBuildStatus == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ResultBuildStatus = nil
	if err = local.L.LoadResultBuildStatus(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ResultBuildStatus == nil {
		t.Error("struct should have been eager loaded")
...
func testBuildLogToOneSetOpBuildStatusUsingResultBuildStatus(t *testing.T) {
	var err error
	...
	for i, x := range []*BuildStatus{&b, &c} {
		err = a.SetResultBuildStatus(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ResultBuildStatus != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ResultBuildLogs[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.Result != x.Status {
			t.Error("foreign key was wrong value", a.Result)
		}

		zero := reflect.Zero(reflect.TypeOf(a.Result))
		reflect.Indirect(reflect.ValueOf(&a.Result)).Set(zero)
		...
		if a.Result != x.Status {
			t.Error("foreign key was wrong value", a.Result, x.Status)
		}
	}
}
...
var (
	buildLogDBTypes = map[string]string{`ID`: `varchar`, `Result`: `varchar`, `StartedAt`: `datetime`, `FinishedAt`: `datetime`, `BranchID`: `varchar`}
	_               = bytes.MinRead
)
