...
func testBuildStatusToManyResultBuildLogs(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ResultBuildLogs().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if v.Result == b.Result {
			bFound = true
		}
		if v.Result == c.Result {
			cFound = true
		}
	}
	...
	if err = a.L.LoadResultBuildLogs(ctx, tx, false, (*[]*BuildStatus)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ResultBuildLogs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ResultBuildLogs = nil
	if err = a.L.LoadResultBuildLogs(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ResultBuildLogs); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testBuildStatusToManyAddOpResultBuildLogs(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddResultBuildLogs(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.Status != first.Result {
			t.Error("foreign key was wrong value", a.Status, first.Result)
		}
		if a.Status != second.Result {
			t.Error("foreign key was wrong value", a.Status, second.Result)
		}

		if first.R.ResultBuildStatus != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.ResultBuildStatus != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.ResultBuildLogs[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ResultBuildLogs[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ResultBuildLogs().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
