...
func testArtifactToOneBuildUsingBuild(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	AddBuildHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Build) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := ArtifactSlice{&local}
	if err = local.L.LoadBuild(ctx, tx, false, (*[]*Artifact)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testArtifactToOneSetOpBuildUsingBuild(t *testing.T) {
	var err error
...
var (
	artifactDBTypes = map[string]string{`ID`: `varchar`, `Size`: `bigint`, `CreatedAt`: `datetime`, `DeletedAt`: `datetime`, `BuildID`: `varchar`}
	_               = bytes.MinRead
)
