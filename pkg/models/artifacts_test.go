...
func testArtifactToManyStaticSiteDetails(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, artifactDBTypes, true, artifactColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Artifact struct: %s", err)
	}
	...
	if err = randomize.Struct(seed, &b, staticSiteDetailDBTypes, false, staticSiteDetailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, staticSiteDetailDBTypes, false, staticSiteDetailColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	queries.Assign(&b.ArtifactID, a.ID)
	queries.Assign(&c.ArtifactID, a.ID)
	...
	check, err := a.StaticSiteDetails().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.ArtifactID, b.ArtifactID) {
			bFound = true
		}
		if queries.Equal(v.ArtifactID, c.ArtifactID) {
			cFound = true
		}
	}
	...
	slice := ArtifactSlice{&a}
	if err = a.L.LoadStaticSiteDetails(ctx, tx, false, (*[]*Artifact)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.StaticSiteDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.StaticSiteDetails = nil
	if err = a.L.LoadStaticSiteDetails(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.StaticSiteDetails); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testArtifactToManyAddOpStaticSiteDetails(t *testing.T) {
	var err error
	...
	foreigners := []*StaticSiteDetail{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, staticSiteDetailDBTypes, false, strmangle.SetComplement(staticSiteDetailPrimaryKeyColumns, staticSiteDetailColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*StaticSiteDetail{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddStaticSiteDetails(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.ArtifactID) {
			t.Error("foreign key was wrong value", a.ID, first.ArtifactID)
		}
		if !queries.Equal(a.ID, second.ArtifactID) {
			t.Error("foreign key was wrong value", a.ID, second.ArtifactID)
		}

		if first.R.Artifact != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Artifact != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.StaticSiteDetails[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.StaticSiteDetails[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.StaticSiteDetails().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testArtifactToManySetOpStaticSiteDetails(t *testing.T) {
	var err error
	...
	err = a.SetStaticSiteDetails(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetStaticSiteDetails(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.ArtifactID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.ArtifactID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.ArtifactID) {
		t.Error("foreign key was wrong value", a.ID, d.ArtifactID)
	}
	if !queries.Equal(a.ID, e.ArtifactID) {
		t.Error("foreign key was wrong value", a.ID, e.ArtifactID)
	}
	...
	if a.R.StaticSiteDetails[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.StaticSiteDetails[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testArtifactToManyRemoveOpStaticSiteDetails(t *testing.T) {
	var err error
	...
	err = a.AddStaticSiteDetails(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveStaticSiteDetails(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.StaticSiteDetails) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.StaticSiteDetails[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.StaticSiteDetails[0] != &e {
